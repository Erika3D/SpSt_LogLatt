...
function LLNS_Regular_IC(Re,nseed)

% Solver for the three-dimensional Landau–Lifshitz Navier–Stokes (LLNS)
% equations with Regular initial condition on the golden logarithmic lattice.
%
...
alpha = 15/4; % Scaling exponent controlling the fluctuation amplitude.
theta = 10^(-8); % Dimensionless temperature.
...
dirname = ['Outputs_PDF/Regular_IC/Re=',num2str(Re),'/Sim_',num2str(nseed),'/'];
...
T = 0:h:0.2; % Time interval and fixed time-step size.

nt1 = ceil(0.075/h) +1; % Closest grid point at or above t = 0.075.
t = 0; % Initial time.
...
for nk=2:length(T)
    ...
    % large-scale wave vector (\lambda,1,1) and a the fixed time t = 0.075.
    if nk == nt1
        w1 = real(wn(2,1,1,1,1));
    end
end

% Save first component of the vorticity field, evaluated at a fixed 
% large-scale wave vector (\lambda,1,1) and a the fixed time t = 0.2.
w2 = real(wn(2,1,1,1,1));
save([dirname,'w_t.mat'],'w1','w2');
